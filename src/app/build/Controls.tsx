    ...
    <div className={clsx(`flex flex-col gap-6`, className)}>
      ...
      <Options title="Wheels" selectedName={selectedWheel?.uid}>
        {wheels.map((wheels) => (
          <Option
            key={wheels.uid}
            imageField={wheels.texture}
            imgixParams={{
              rect: [20, 10, 850, 850],
              width: 150,
              height: 150,
            }}
            selected={wheels.uid === selectedWheel?.uid}
            onClick={() => setWheel(wheels)}
          >
            {wheels.uid?.replace(/-/g, " ")}
          </Option>
        ))}
      </Options>
      <Options title="Trucks" selectedName={selectedTruck?.uid}>
        {metals.map((metals) => (
          <Option
            key={metals.uid}
            colorField={metals.color}
            selected={metals.uid === selectedTruck?.uid}
            onClick={() => setTruck(metals)}
          >
            {metals.uid?.replace(/-/g, " ")}
          </Option>
        ))}
      </Options>
      <Options title="Bolts" selectedName={selectedBolt?.uid}>
        {metals.map((metals) => (
          <Option
            key={metals.uid}
            colorField={metals.color}
            selected={metals.uid === selectedBolt?.uid}
            onClick={() => setBolt(metals)}
          >
            {metals.uid?.replace(/-/g, " ")}
          </Option>
        ))}
